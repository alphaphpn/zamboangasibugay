<?php
	...
	$mdate = date("Y-m", strtotime(isset($_GET['mdate']) ? $_GET['mdate'] : date("Y-m")));
?>
...
<main class="page-content">
	<div class="container-fluid bg-light-opacity">
		<div class="d-flex">
			<h4 class="mr-2 mb-2">Montly Report</h4>
			<a href="<?php echo $dirbak.'routes/reports/monthly/print/?mdate='.$mdate; ?>" class="btn btn-large btn-primary">Print</a>
		</div>

		<div class="d-flex">
			<div class="ml-auto">
				<form method="_GET">
					<label for="mdate">Month and Year:</label>
		...
		<?php
			...
			$qry_sbtotal = "SELECT SUM(sub_total) AS subtotal, COUNT(*) AS total_rows, SUM(sub_total_qty) AS total_qty, SUM(sub_total_item) AS total_item FROM tbl_order_customer WHERE status=:status AND deleted=0 AND YEAR(date(created))=:myear AND MONTH(date(created))=:mmonth";
			$stmt_sbtotal = $cnn_sbtotal->prepare($qry_sbtotal);
			$status = 'Paid';
			$hjyear = date("Y", strtotime($mdate));
			$hjmonth = date("m", strtotime($mdate));
			$stmt_sbtotal->bindParam(':status', $status);
			$stmt_sbtotal->bindParam(':myear', $hjyear);
			$stmt_sbtotal->bindParam(':mmonth', $hjmonth);
			...
			<table id="listRecView" class="table table-striped table-hover">
				<thead>
					<tr>
						...
						<th class="d-none"></th>
						<th></th>
						<th>Filtered<br>Year & Month:<br><h5 class="text-danger"><?php echo $mdate; ?></h5></th>
						<th></th>
						<th></th>
					</tr>
					<tr>
				...
				<tbody>
					<?php
						...
						$qry = "SELECT * FROM {$tblname} WHERE status=:status AND deleted=0 AND YEAR(date(created))=:myear1 AND MONTH(date(created))=:mmonth1 ORDER BY {$prim_id} DESC";
						$stmt = $cnn->prepare($qry);
						$status = 'Paid';
						$hjyear2 = date("Y", strtotime($mdate));
						$hjmonth2 = date("m", strtotime($mdate));
						$stmt->bindParam(':status', $status);
						$stmt->bindParam(':myear1', $hjyear2);
						$stmt->bindParam(':mmonth1', $hjmonth2);
						$stmt->execute();
						$xno = 0;
